...
const ImagePromoBlock = ({heading, body, cta, image, reverse, textColor, imagePos}) => {
    ...
    return (
        <div className=" flex flex-col md:grid grid-cols-1 grid-rows-2 md:grid-cols-2 md:grid-rows-1 lg:h-[50rem]">
            
            <div className={`h-full ${!reverse && `md:order-2`}`}> 
                <img src={image} className={`object-cover h-full w-full ${imagePos}`}/>

            </div>
            
            <div className={`px-5 py-20 flex flex-col items-center justify-start md:items-start md:order-1 md:justify-center ${reverse ? 'md:pl-10' : 'md:pr-10'} text-${textColor}`}>
