...
const Button = ({ children, fn, url, target, variation, bgColor, color }) => {
    ...
    return (
        <>
            {url ?
                <a href={url} target={target} className={`pointer underline text-xl hover:opacity-80 btn-${variation}`} style={{ ...(bgColor && { backgroundColor: bgColor }), ...(color && { color: color }) }}>{children}</a>
                :
